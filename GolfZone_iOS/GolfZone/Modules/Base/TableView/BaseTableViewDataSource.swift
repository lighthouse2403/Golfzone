...
class BaseTableViewDataSource<TableViewCell: BaseTableViewCell>: NSObject, UITableViewDelegate, UITableViewDataSource {
    ...
    func addNoDataView(icons: String = "ico_nodata") {
        ...
        if emptyType == .normal {
                self.tableView.setNoDataView(content: noDataMessage)
        } else {
        }
    }
    ...
    @objc
    func didSelectedHeader(gesture: UITapGestureRecognizer) {
